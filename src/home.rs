use crate::{get_api_request, net, helpers::V4_API_URL};
use aidoku::{
    HomeComponent, HomeLayout, HomePartialResult, 
    Listing, ListingKind, Manga, 
    MangaStatus, MangaWithChapter,
    Result,
...
pub fn get_home_layout() -> Result<HomeLayout> {
    ...
    if let Ok(resp) = r0
        && let Ok(categories) = resp.get_json_owned::<Vec<crate::models::RecommendCategory>>() {
                for cat in categories {
                    let manga_list: Vec<Manga> = cat.data.iter()
                    ...
                    if manga_list.is_empty() { continue; }
    ...
    if let Ok(resp) = r1
        && let Ok(response) = resp.get_json_owned::<crate::models::ApiResponse<crate::models::FilterData>>()
            && let Some(data) = response.data {
            latest_entries = data.comic_list.into_iter()
                .map(|item| item.into_manga_with_chapter())
                .collect();
        }

    fn parse_rank_page(resp: Response) -> Vec<Manga> {
        if let Ok(response) = resp.get_json_owned::<crate::models::ApiResponse<Vec<crate::models::RankItem>>>()
    ...
    if let Ok(resp) = r2 { hot_entries.extend(parse_rank_page(resp)); }
    ...
    fn parse_audience_scroller(resp: Response) -> Vec<aidoku::Link> {
        if let Ok(response) = resp.get_json_owned::<crate::models::ApiResponse<crate::models::FilterData>>()
            && let Some(data) = response.data {
                return crate::models::manga_list_from_filter(data.comic_list).entries.into_iter()
                    .map(|manga| {
                        aidoku::Link {
                            title: manga.title.clone(),
