...
impl Source for Hennoveltranslations {
    ...
    fn get_manga_update(
        ...
        if needs_details {
            manga.title = html.select("h1").and_then(|el| el.text()).unwrap_or_default();
        ...
        if needs_chapters {
            ...
            if let Some(free_list) = html.select(".episode-list2") {
                if let Some(links) = free_list.select("a") {
                    for node in links {
                        if let Some(chapter_url) = node.attr("href") {
                            if chapter_url.contains("/archives/episodes/") {
    ...
    fn get_page_list(&self, _manga: Manga, chapter: Chapter) -> Result<Vec<Page>> {
        ...
        if let Some(content) = html.select(".episode-content") {
            if let Some(elements) = content.select("p") {
                for p in elements {
        ...
        if paragraphs.len() <= 1 && !subheading.is_empty() {
            if let Some(content) = html.select(".entry-content, .reading-content") {
                if let Some(elements) = content.select("p") {
                    for p in elements {
                        let text = p.text().unwrap_or_default();
                        if !text.is_empty() {
