...
pub fn parse_chapter_list(base_url: String, manga_id: String) -> Result<Vec<Chapter>> {
	...
	while page <= last_page {
		...
		for chapter in chapters {
			...
			let chapter_name = chapter.get("chapter_name").as_string()?.read();
			let title = chapter
				.get("chapter_title")
				.as_string()
			...
			let index = chapter
				.get("index")
				.as_string()
				.map(|value| value.read().parse::<f32>().unwrap_or(-1.0))
				.unwrap_or(-1.0);
			...
			all_chapters.push(Chapter {
				id,
				title: if title.is_empty() { chapter_name } else { title },
				chapter: index,
				date_updated,
				url,
				..Default::default()
...
pub fn parse_page_list(
	...
	for page in obj.select("link[rel=preload][as=image]").array() {
		let obj = page.as_node().expect("node array");
		let url = obj.attr("href").read();

		if !url.contains("/uploads/series/") {
			continue;
		}

		page_list.push(Page { url, ..Default::default() });
	}

	for (index, page) in page_list.iter_mut().enumerate() {
		page.index = index as i32;
	}
