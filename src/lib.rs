...
use helpers::{get_api_request, V4_API_URL};
...
impl Source for Zaimanhua {
    ...
    fn get_manga_update(
        ...
        let response: models::ApiResponse<models::DetailData> = get_api_request(&url)?.json_owned()?;
        ...
        let manga_detail = detail_data.data.ok_or_else(|| error!("Missing nested data (API error?)"))?;
    ...
    fn get_page_list(&self, manga: Manga, chapter: Chapter) -> Result<Vec<Page>> {
        ...
        let response: models::ApiResponse<models::ChapterData> = get_api_request(&url)?.json_owned()?;
        let chapter_data = response.data.ok_or_else(|| error!("Missing data"))?;
        let page_data = chapter_data.data;
        
        let page_urls = page_data.page_url_hd
            .or(page_data.page_url)
            .ok_or_else(|| error!("Missing page_url"))?;
        
        let pages = page_urls.into_iter()
            .map(|url| Page {
    ...
    fn handle_deep_link(&self, url: String) -> Result<Option<DeepLinkResult>> {
        ...
        if url.contains("/chapter/") {
             ...
             if let Some(start) = url.find("/chapter/") {
                 let parts: Vec<&str> = url[start + 9..].split('/').filter(|s| !s.is_empty()).collect();
                 if parts.len() >= 2 {
                     let comic_id = parts[0];
                     let chapter_id = parts[1];
                     
                     if comic_id.chars().all(|c| c.is_ascii_digit()) && chapter_id.chars().all(|c| c.is_ascii_digit()) {
...
impl DynamicSettings for Zaimanhua {
    fn get_dynamic_settings(&self) -> Result<Vec<Setting>> {
        ...
        let user_info_opt = if is_logged_in {
            if let Some(token) = settings::get_token() {
                if let Ok(info_data) = net::get_user_info(&token) {
                    info_data.user_info
                } else {
                    None
                }
        ...
        account_items.push(
            LoginSetting {
                key: "login".into(),
                title: "登录".into(),
                notification: Some("logout".into()),  // This fires on logout (and potentially login)
        ...
        if let Some(user_info) = user_info_opt {
            // Extract info
            let username = user_info.username.as_deref()
...
impl ListingProvider for Zaimanhua {
    fn get_manga_list(&self, listing: Listing, page: i32) -> Result<MangaPageResult> {
        ...
        if listing.id == "rank-monthly" {
            ...
            let data = response.data.ok_or_else(|| aidoku::error!("No data in rank response"))?;
        ...
        let url = match listing.id.as_str() {
            ...
            "subscribe" => {
                ...
                let response: models::ApiResponse<models::SubscribeData> = net::auth_request(&url, &token)?.json_owned()?;
                let data = response.data.ok_or_else(|| aidoku::error!("Invalid subscribe response"))?;
        ...
        let data = response.data.ok_or_else(|| aidoku::error!("No data in filter response"))?;
