...
pub fn login(username: &str, password: &str) -> Result<Option<String>> {
    ...
    let response: models::ApiResponse<models::LoginData> = post_request(&url)?
        .body(body.as_bytes())
        .json_owned()?;
...
pub fn get_user_info(token: &str) -> Result<models::UserInfoData> {
    ...
    let response: models::ApiResponse<models::UserInfoData> = auth_request(&url, token)?.json_owned()?;
    response.data.ok_or_else(|| aidoku::error!("Missing user info data"))
}
