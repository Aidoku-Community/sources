...
use aidoku::{
	...
	prelude::*,
	Manga, MangaPageResult, MangaStatus, Viewer, Result,
};
...
impl MangaPage for Document {
	fn update_details(&self, manga: &mut Manga) -> Result<()> {
		...
		manga.tags = Some(categories.clone());
		manga.status = status;
		manga.url = Some(url);

		// Set viewer based on categories
		manga.viewer = if categories.iter().any(|tag| tag.contains("国漫") || tag.contains("韩漫")) {
			Viewer::Webtoon
