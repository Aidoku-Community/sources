use aidoku::{
    Chapter, Manga, MangaPageResult, MangaStatus, MangaWithChapter, ContentRating,
...
// === Search API ===
// Response: data.list[] with id, comic_id(=0), title, authors, cover, status
...
// === Filter API ===
// Response: data.comicList[] with id, name, authors, cover, status
...
impl FilterItem {
    pub fn into_manga_with_chapter(self) -> MangaWithChapter {
        let manga: Manga = self.clone().into();
...
impl MangaDetail {
    pub fn into_manga(self, key: String) -> Manga {
        let authors: Option<Vec<String>> = self.authors.map(|list| {
            list.into_iter()
        ...
        if let Some(groups) = self.chapters {
            for group in groups {
                ...
                for item in group.data {
                    ...
                    let url = Some(format!(
                        "https://manhua.zaimanhua.com/details/{}/{}",
                        manga_id, chapter_id
                    ));
...
pub fn manga_list_from_search(items: Vec<SearchItem>) -> MangaPageResult {
    let entries: Vec<Manga> = items.into_iter()
