...
impl LibraryManga {
    pub fn to_manga(&self, base_url: &str) -> Manga {
        Manga {
            ...
            cover: self.image.clone().map(|s| s.into()),
            url: Some(format!("{}/manhwa/{}", base_url, self.id)),
            status: self.status.as_ref().map(|s| match s.as_str() {
...
impl SeeResponse {
    pub fn parse_manga(&self, base_url: &str) -> Manga {
        Manga {
            ...
            tags: self.categories.as_ref().map(|cats: &Vec<serde_json::Value>| {
                cats.iter()
                    .filter_map(|cat: &serde_json::Value| {
                        cat.as_object()
                            .and_then(|obj: &serde_json::Map<String, serde_json::Value>| obj.values().next())
